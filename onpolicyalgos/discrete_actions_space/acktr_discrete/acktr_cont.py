...
def rollout(env, policy, max_pathlength, animate=False, obfilter=None):
    ...
    for _ in range(max_pathlength):
        ...
        obs.append(state)
        ac, logit, logp = policy.act(state)
        acs.append(ac)
        logits.append(logit)
        logps.append(logp)
        prev_ob = np.copy(ob)
    ...
    return {"observation" : np.array(obs), "terminated" : terminated,
            ...
            "logits": np.array(logits), "logp" : np.array(logps)}
...
def learn(env, policy, vf, gamma, lam, timesteps_per_batch, num_timesteps,
    ...
    while True:
        ...
        oldlogit = np.concatenate([path["logits"] for path in paths])
        ...
        # Adjust stepsize
        kl = policy.compute_kl(ob_no, oldlogit)
        if kl > desired_kl * 2:
